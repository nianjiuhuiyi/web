...
def show():
    ...
    # 修改了一下查询的表，然后把下面的表结构也做了删减
    query_sql = "select * from province;"
    ...
    """
    # 这每行的内容 (注意这里的 <a href='show/%s.py'> ，这里有个 %s 代表每个省份具体的数字)
    line = """
        <tr>
            <td>%s</td>
            <td>%s</td>
...
# 注意点.前面的反斜杠，一定要，后面正则的时候影响很大
@route("/show/(\d+)\.py")  # 从展示的主页面收藏数据
def collect_add(ret):
    "ret是传进来的正则匹配结果(且一定是匹配到的，没配到的进不来).group(1)就是对应的各省份的district_id "
    ...
    cursor.execute(query_sql, (district_id, ))  # 注意写元祖进去，不要自己在上面.format()拼接，小心sql注入
    query_data = cursor.fetchall()

    # (1)判断这个收藏的id是否在数据库的范围内，若不在就没有数据，就是认为模拟的数据：
    if not query_data:
        results = "行行好，大哥手下留情，不要再攻击我们了..."
    else:
        # (2)如果数据合法，还要判断是否已经收藏了
...
def collect_del(ret):
    ...
    # 注意：这里返回收藏页面，是以 :8899/collect.py 作为相对路径的，
    # - href=".." 就回到了 :8899/
...
def application(env: dict, start_response):
    ...
    if file_name.endswith(".py"):
        ...
        try:
            # 这是为了匹配,是@route("/show/(\d+)\.py")这个格式再把正则匹配的结果传家进去，否则是没有参数的
            # 这种一般传进来的是 /show/123.py
